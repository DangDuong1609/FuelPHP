...
taoKetNoi($link);
    if(isset($_POST['submit'])){
        ...
        if($error===0){
            if($img_size > 2000000){
                $em="File quá lớn";
                header("Location: profile.php?idaccount=$account_uuid&massedit=$em");
            }
            else{
                ...
                $username_uuid = $row_account_usid['username'];
                    ...
                    elseif (password_verify($passwordncu, $password)) {
                        if ($passwordnew === $passwordsuccessnew) {
                            ...
                            header("Location: profile.php?idaccount=$account_uuid&massedit=$em");die();
                        }
                    }
                    else {
                        $em="Password Cũ Sai";
                ...
                if(in_array($img_ex_lc,$allowed_exs)){
                    ...
                    $result=chayTruyVanKhongTraVeDL($link,"UPDATE account SET full_name='$fullname', email='$email',username= '$username_uuid', password='$password_hash',img='$name_image_save', created_at='$created_at',updated_at='$updated_at'  WHERE usid= '$account_uuid'");

                    if($result){
                        $em="Edit thành công";
                        header("Location: profile.php?idaccount=$account_uuid&messsanpham=$em");
                    }
                    else{
                        $em="Edit thất bại";
                        header("Location: profile.php?idaccount=$account_uuid&massedit=$em");
            ...
            $result=chayTruyVanKhongTraVeDL($link,"UPDATE account SET full_name='$fullname', email='$email',username='$username_uuid', password='$password_hash', img='$new_img_name', created_at='$created_at',updated_at='$updated_at'  WHERE usid= '$account_uuid'");
